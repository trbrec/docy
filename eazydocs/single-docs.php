<?php
/**
 * EazyDocs single-docs template override for Docy theme.
 *
 * Mirrors the plugin's single-docs.php exactly. The only difference is the
 * banner block, which is replaced with Docy's banner source control.
 *
 * To update: re-sync everything from the plugin template EXCEPT the banner
 * block marked below.
 */
...
$credit_text = ezd_get_opt( 'eazydocs-credit-text', sprintf( __( 'Powered By %1$s EazyDocs %2$s', 'eazydocs' ), '<a href="https://wordpress.org/plugins/eazydocs/" target="_blank" rel="noopener noreferrer">', '</a>' ) );
...
/* -------------------------------------------------------------------------
 * BANNER BLOCK — only this block differs from the plugin template.
 *
 * Reads "Docs Banner Source" from Docy Theme Settings → Header → Search Banner:
 *   'eazydocs' (default) → EazyDocs' own banner (search-banner.php or custom-banner.php).
 *   'docy'               → Docy theme's styled search/hero banner.
 * ------------------------------------------------------------------------- */
$docs_banner_source = function_exists( 'docy_opt' ) ? docy_opt( 'docs_banner_source', 'eazydocs' ) : 'eazydocs';

if ( $docs_banner_source === 'docy' && function_exists( 'docy_search_banner' ) ) {
	get_template_part(
		'template-parts/header-elements/search-banner/sbnr',
		docy_search_banner()
	);
} elseif ( defined( 'EZD_PATH' ) ) {
	$plugin_banner = $single_layout === 'default'
		? EZD_PATH . '/templates/search-banner.php'
		: EZD_PATH . '/templates/custom-banner.php';
	if ( file_exists( $plugin_banner ) ) {
		// EazyDocs Banner manages its own keywords from EazyDocs settings.
		// Do not inject Docy's keyword template here.
		load_template( $plugin_banner, false );
	}
}
/* ----------------------------------------------------------------------- */
?>
