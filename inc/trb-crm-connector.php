...
function trb_crm_connector_demo_payload( $request_id ) {
	...
	return trb_crm_connector_clean( array(
		...
		'review_context' => is_array( $payload['review_context'] ?? null ) ? array_intersect_key( $payload['review_context'], array_flip( array( 'version', 'focus', 'lyrics', 'music', 'performance', 'notes' ) ) ) : array(),
		...
		'submitted_at' => ! empty( $payload['submitted_at'] ) ? ( is_numeric( $payload['submitted_at'] ) ? (int) $payload['submitted_at'] : (int) strtotime( $payload['submitted_at'] ) ) : strtotime( $post->post_date_gmt . ' UTC' ),
