...
function trb_owner_dashboard_demo_summary( $review ) {
	if ( is_string( $review ) && trim( $review ) !== '' ) return wp_trim_words( wp_strip_all_tags( $review ), 30 );
...
function trb_owner_dashboard_render() {
	...
	<div class="wrap trb-owner-dashboard">
		...
		<details><summary><strong>Dettagli tecnici, copyright e file (<?php echo esc_html( count( $detail_files ) ); ?>)</strong></summary><pre><?php echo esc_html( wp_json_encode( array( 'decisione' => $detail_decision, 'tecnica' => $detail_technical, 'file' => $detail_files, 'archivio_pcloud' => get_post_meta( $selected_release->ID, '_trb_release_pcloud_archive', true ), 'ricezione' => get_post_meta( $selected_release->ID, '_trb_release_intake_phase', true ), 'file_locali_presenti' => array_map( static function( $file ) { $path = function_exists( 'trb_release_pcloud_local_file' ) ? trb_release_pcloud_local_file( $file ) : ''; return array( 'nome' => $file['name'] ?? '', 'presente' => $path && is_file( $path ) ); }, $detail_files ) ), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ) ); ?></pre></details>
		<?php endif; ?>
