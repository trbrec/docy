...
require_once __DIR__ . '/trb-symphonic-credits.php';
require_once __DIR__ . '/trb-symphonic-genres.php';
/**
...
function trb_portal_contributor_roles() {
	// Canonical portal credit roles, also rendered into the browser datalist.
	// Authors/composers retain their separate publishing fields.
	$credits = array( 'A&R', 'Accordion', 'Acoustic Guitar', 'Actor', 'Additional Engineer', 'Additional Production', 'Arranger', 'Art Direction', 'Assistant Engineer', 'Assistant Mastering Engineer', 'Assistant Mixing Engineer', 'Assistant Producer', 'Audio Editor', 'Background Vocalist', 'Bandoneon', 'Banjo', 'Bass', 'Bass Clarinet', 'Bass Guitar', 'Bass Trombone', 'Bass Vocals', 'Bassoon', 'Beat Maker', 'Bells', 'Bongos', 'Bouzouki', 'Brass', 'Cabasa', 'Castanets', 'Casting', 'Celesta', 'Cello', 'Choir', 'Choir Director', 'Clarinet', 'Clavinet', 'Co-Producer', 'Composer', 'Concertina', 'Conductor', 'Congas', 'Contrabass', 'Contrabassoon', 'Contractor', 'Coordinator', 'Copyist', 'Cornet', 'Creative Director', 'Cuatro', 'Cymbals', 'DJ', 'Dialogue Editor', 'Digital Editing', 'Djembe', 'Dobro', 'Drum Machine', 'Drum Programming', 'Drums', 'Editing', 'Editor', 'Electric Guitar', 'Engineer', 'Euphonium', 'Executive Producer', 'FOH Engineer', 'Featuring', 'Fiddle', 'Field Recording', 'Flugelhorn', 'Flute', 'French Horn', 'Glockenspiel', 'Gong', 'Graphic Design', 'Guitar', 'Hammond Organ', 'Handclaps', 'Harmonica', 'Harp', 'Harpsichord', 'Horn', 'Illustration', 'Immersive Audio Engineer', 'Immersive Audio Mixing Engineer', 'Instrumentation', 'Keyboards', 'Lap Steel Guitar', 'Lead Guitar', 'Lead Vocalist', 'Librettist', 'Live Engineer', 'Location Recording', 'Lyricist', 'Management', 'Mandolin', 'Marimba', 'Mastering Engineer', 'Mellotron', 'Melodica', 'Mixing Assistant', 'Mixing Engineer', 'Music Director', 'Music Supervisor', 'Musical Assistance', 'Oboe', 'Orchestra', 'Orchestral Engineer', 'Orchestrator', 'Organ', 'Pedal Steel Guitar', 'Percussion', 'Performer', 'Photography', 'Piano', 'Piccolo', 'Post Production', 'Producer', 'Production Assistant', 'Programming', 'Recorder', 'Recording Assistant', 'Recording Engineer', 'Reissue Producer', 'Remixer', 'Restoration', 'Rhodes Piano', 'Saxophone', 'Score Editor', 'Sequencer', 'Session Coordinator', 'Sitar', 'Slide Guitar', 'Snare Drum', 'Sound Design', 'Sound Engineer', 'Steel Guitar', 'Strings', 'Studio Personnel', 'Surround Mixing Engineer', 'Synth Bass', 'Synthesizer', 'Tabla', 'Tambourine', 'Technician', 'Tenor Saxophone', 'Theremin', 'Timpani', 'Tracking Engineer', 'Translation', 'Trombone', 'Trumpet', 'Tuba', 'Turntables', 'Ukulele', 'Upright Bass', 'Vibraphone', 'Viola', 'Violin', 'Visuals', 'Vocal Arranger', 'Vocal Editing', 'Vocal Engineer', 'Vocal Producer', 'Vocalist', 'Vocals', 'Whistle', 'With', 'Woodwinds', 'Writer', 'Xylophone' );
	return array(
...
/** Return every requirement with the exact artist-facing label and owning module. */
function trb_portal_artist_profile_requirements( $user_id = 0 ) {
	$user_id = $user_id ? $user_id : get_current_user_id();
	$user = get_userdata( $user_id );
	$requirements = array();
	$add = static function( $key, $label, $group, $complete ) use ( &$requirements ) {
		$requirements[] = compact( 'key', 'label', 'group', 'complete' );
	};
	$add( 'first_name', 'Nome anagrafico', 'contract', $user && '' !== trim( (string) $user->first_name ) );
	$add( 'last_name', 'Cognome anagrafico', 'contract', $user && '' !== trim( (string) $user->last_name ) );
	$add( 'email', 'E-mail di riferimento', 'contract', $user && '' !== trim( (string) $user->user_email ) );
...
function trb_portal_release_file_url( $release_id, $file_index, $inline = false, $stream = false ) {
	$url = add_query_arg(
		array( 'action' => 'trb_portal_release_file', 'release_id' => absint( $release_id ), 'file_index' => absint( $file_index ), 'view' => $inline ? '1' : '0', 'stream' => $stream ? '1' : '0' ),
...
function trb_portal_serve_release_file() {
	...
	$stream = ! empty( $_GET['stream'] ) && ( $file['kind'] ?? '' ) === 'audio';
	if ( $stream ) {
		$size = (int) filesize( $target );
		$start = 0;
		$end = $size - 1;
		header( 'Accept-Ranges: bytes' );
		header( 'Content-Disposition: inline; filename="' . sanitize_file_name( $file['name'] ?? 'audio.wav' ) . '"' );
		if ( ! empty( $_SERVER['HTTP_RANGE'] ) && preg_match( '/bytes=(\d*)-(\d*)/', sanitize_text_field( wp_unslash( $_SERVER['HTTP_RANGE'] ) ), $range ) ) {
			$start = '' !== $range[1] ? max( 0, (int) $range[1] ) : 0;
			$end = '' !== $range[2] ? min( $end, (int) $range[2] ) : $end;
			if ( $start > $end || $start >= $size ) { status_header( 416 ); header( 'Content-Range: bytes */' . $size ); exit; }
			status_header( 206 );
			header( 'Content-Range: bytes ' . $start . '-' . $end . '/' . $size );
		}
		header( 'Content-Length: ' . ( $end - $start + 1 ) );
		$handle = fopen( $target, 'rb' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen
		fseek( $handle, $start );
	...
	$previewable = in_array( sanitize_mime_type( $file['type'] ?? '' ), array( 'application/pdf', 'image/jpeg', 'image/png', 'image/webp', 'text/plain' ), true );
	$inline = ! empty( $_GET['view'] ) && $previewable && ( $file['kind'] ?? '' ) !== 'audio';
...
/** Artists cannot replace definitive materials after TRB has approved a release. */
function trb_portal_release_files_are_locked( $release_id ) {
	$release_id = absint( $release_id );
	if ( ! $release_id || 'trb_release' !== get_post_type( $release_id ) ) return false;
	$pipeline = sanitize_key( get_post_meta( $release_id, '_trb_release_pipeline_status', true ) );
	$contract = sanitize_key( get_post_meta( $release_id, '_trb_contract_state', true ) );
	return 'approved' === $pipeline || in_array( $contract, array( 'preparing', 'contract_sent', 'signed' ), true );
}
...
function trb_portal_replace_release_file() {
	...
	if ( trb_portal_release_files_are_locked( $release_id ) && ! current_user_can( 'manage_options' ) ) {
...
function trb_portal_normalize_release_draft_pairs( $pairs, &$report = null ) {
	...
	foreach ( array_slice( $pairs, 0, 4000 ) as $pair ) {
		...
		if ( preg_match( '/^trb_tracks\[(\d+)\]\[credits\]\[(writers|credits|performers|engineering)\]\[(\d+)\]/', $name, $contributor_match ) ) {
...
function trb_portal_start_release() {
	...
	$symphonic = trb_symphonic_release($intake_id);
	$symphonic_genres = trb_symphonic_genre_release($intake_id);
	if ($symphonic_genres && ($genre_errors=trb_symphonic_genre_errors($tracks))) trb_portal_release_submission_response('invalid',implode(' ', $genre_errors),422);
	if ($symphonic && ($credit_errors=trb_symphonic_credit_errors($tracks))) trb_portal_release_submission_response('invalid',implode(' ', $credit_errors),422);
	$tracks = trb_portal_sanitize_release_tracks( $tracks, $symphonic, $symphonic_genres );
	...
	if ( 'previously_released' === $release_state ) {
		...
		foreach ( $tracks as $track_index => &$clean_track ) {
			...
			if ( 'catalogue_reissue' === ( $clean_track['rights_basis'] ?? '' ) ) $clean_track['rights_reference'] = $isrc;
			$seen_isrc[ $isrc ] = true;
		}
		unset( $clean_track );
	}
	foreach ( $tracks as $track ) {
		if ( 'original' !== ( $track['content_nature'] ?? '' ) ) continue;
		if ( 'catalogue_reissue' === ( $track['rights_basis'] ?? '' ) && 'previously_released' !== $release_state ) {
			trb_portal_release_submission_response( 'invalid', 'La dichiarazione di ripubblicazione è utilizzabile soltanto per una release già pubblicata.', 422 );
		}
		if ( 'owned' === ( $track['rights_basis'] ?? '' ) && 'unreleased' !== $release_state ) {
			trb_portal_release_submission_response( 'invalid', 'Per una release già pubblicata seleziona la dichiarazione relativa alla versione già distribuita.', 422 );
		}
	}
...
/**
 * Handle the JSON release request before admin redirect plugins can replace
 * its body with an HTML response. This also repairs retries from a form tab
 * that was opened before the dedicated AJAX URL was deployed.
 */
function trb_portal_route_release_ajax_request() {
	if ( 'POST' !== strtoupper( isset( $_SERVER['REQUEST_METHOD'] ) ? sanitize_text_field( wp_unslash( $_SERVER['REQUEST_METHOD'] ) ) : '' ) ) return;
	$action = isset( $_POST['action'] ) ? sanitize_key( wp_unslash( $_POST['action'] ) ) : '';
	$ajax   = isset( $_POST['trb_release_ajax'] ) ? sanitize_text_field( wp_unslash( $_POST['trb_release_ajax'] ) ) : '';
	if ( 'trb_portal_start_release' !== $action || '1' !== $ajax ) return;
	if ( function_exists( 'trb_release_bridge_capture_isrc' ) ) trb_release_bridge_capture_isrc();
	trb_portal_start_release();
}
add_action( 'admin_init', 'trb_portal_route_release_ajax_request', 20 );
...
function trb_portal_sanitize_writers( $rows, $symphonic = false ) {
	$clean = array();
	foreach ( (array) $rows as $row ) {
		$name       = isset( $row['name'] ) ? sanitize_text_field( $row['name'] ) : '';
		$roles      = isset( $row['roles'] ) ? array_map( 'sanitize_text_field', (array) $row['roles'] ) : array();
		$roles      = array_values( array_intersect( $symphonic ? array_keys(trb_symphonic_roles()['writers']) : array( 'Lyricist', 'Composer' ), array_unique( $roles ) ) );
		if ( '' !== $name && ! empty( $roles ) ) {
...
function trb_portal_sanitize_release_tracks( $tracks, $symphonic = false, $symphonic_genres = false ) {
	$genres    = trb_portal_genres();
	$genre_catalogue = $symphonic_genres ? trb_symphonic_genres() : array();
	...
	foreach ( $tracks as $track ) {
		...
		$writers   = trb_portal_sanitize_writers( isset( $credits['writers'] ) ? $credits['writers'] : array(), $symphonic );
		$additional_credits = trb_portal_sanitize_contributors( isset( $credits['credits'] ) ? $credits['credits'] : array(), $roles['credits'] );
		$performers = $engineering = array();
		if ($symphonic) {
			if (trb_symphonic_credit_errors(array($track))) continue;
			$catalogue=trb_symphonic_roles();
			$performers=trb_portal_sanitize_contributors($credits['performers'] ?? array(),$catalogue['performers']);
			$engineering=trb_portal_sanitize_contributors($credits['engineering'] ?? array(),$catalogue['engineering']);
			$additional_credits=array_merge($performers,$engineering);
		}
		...
		$allowed_rights = array( 'original' => array( 'owned', 'catalogue_reissue' ), 'type_beat' => array( 'exclusive', 'nonexclusive' ), 'remix' => array( 'licensed' ), 'protected_samples' => array( 'licensed' ) );
		if (
			'' === $title || '00:00' === $duration || ! preg_match( '/^[0-9]{2}:[0-5][0-9]$/', $duration ) ||
			( $symphonic_genres ? ( ! isset( $genre_catalogue[ $primary ] ) || ! in_array( $secondary, $genre_catalogue[ $primary ], true ) ) : ( ! in_array( $primary, $genres, true ) || ( '' !== $secondary && ( $secondary === $primary || ! in_array( $secondary, $genres, true ) ) ) ) ) ||
		...
		if ($symphonic) {
			$key=count($clean)-1;
			$clean[$key]['credit_schema']='symphonic-2026-09';
			$clean[$key]['credits']['performers']=$performers;
			$clean[$key]['credits']['engineering']=$engineering;
			$clean[$key]['credits']['producers']=$summary($engineering);
			$clean[$key]['credits']['musicians']=$summary($performers);
		}
	}
	return array_values( $clean );
}
...
function trb_portal_seed_guides() {
	...
	$guides['dds-limite-release'] = array(
		...
		'content'  => '<p>Il servizio <strong>DDS comprende 12 release nell’arco di un anno</strong>. Nella configurazione attuale è proposto come abbonamento da 49 euro al mese e consente una release per mese solare.</p><ul><li>Ogni pratica completata consuma una release, indipendentemente dalla tipologia.</li><li>Bozze e caricamenti non completati non consumano la disponibilità.</li><li>La quota si rinnova il primo giorno del mese secondo il fuso Europe/Rome e non si accumula.</li><li>Per correggere una release usa la stessa pratica; non crearne una seconda.</li></ul><p>Se una pratica deve essere annullata, apri una segnalazione: l’annullamento non ripristina automaticamente la disponibilità.</p>',
	);
	$guides['ddb-limite-release'] = array(
		'title'    => 'DDB CCAD: quante release prevede il tuo contratto',
		'profiles' => array( 'ddb' ),
		'excerpt'  => 'I modelli CCAD 600, 800 e 1200 associati al gruppo DDB prevedono pubblicazioni illimitate.',
		'content'  => '<p>Il gruppo <strong>DDB è associato ai modelli contrattuali CCAD 600, CCAD 800 e CCAD 1200</strong>. Tutti e tre prevedono una quantità illimitata di release durante i 12 mesi contrattuali.</p><p>“Illimitate” non significa che le uscite siano approvate automaticamente o possano essere consegnate senza anticipo: ogni pratica resta subordinata ai requisiti tecnici delle piattaforme, alle tempistiche minime di consegna e alla programmazione concordata.</p><h4>Come verificare la tua regola</h4><p>Apri il contratto associato alla pratica e controlla la voce <strong>Quantità di Pubblicazioni</strong>. Il modello CSAE 600 non appartiene al gruppo DDB, ma al gruppo DDB12. Se il modello mostrato non corrisponde al gruppo del profilo, non creare una nuova pratica e apri una segnalazione.</p>',
	);
		...
		<div class="trb-portal__profile-accordions">
			...
			<details id="trb-profile-identity" class="trb-portal__profile-module" <?php echo empty( $missing_contract ) && $missing_identity ? 'open' : ''; ?>>
				...
				<form class="trb-portal__request-form trb-portal__profile-form" method="post" enctype="multipart/form-data" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
					...
					<label>Biografia artistica aggiornata <span>*</span><input type="file" name="trb_artist_bio_file" accept=".txt,.docx,.odt,.rtf,text/plain,application/rtf,text/rtf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.oasis.opendocument.text" <?php echo empty( $biography_file ) ? 'required' : ''; ?> /><small><?php echo $biography_file ? 'La biografia è già caricata correttamente. Seleziona un nuovo file soltanto quando vuoi aggiornarla: sostituirà quello attuale.' : 'Allega un testo pronto per materiali editoriali, comunicazione e profili ufficiali.'; ?> Formati ammessi: TXT, DOCX, ODT o RTF · massimo 5 MB.</small><?php if ( ! empty( $stored_biography ) && empty( $biography_file ) ) : ?><em class="trb-portal__file-warning">Il file già presente non è in un formato elaborabile a basso costo: scaricalo se necessario e sostituiscilo con TXT, DOCX, ODT o RTF.</em><?php endif; ?></label>
...
function trb_portal_serve_private_file() {
	...
	foreach ( trb_portal_private_profile_files() as $file ) {
		...
		$previewable = in_array( sanitize_mime_type( $file['type'] ), array( 'application/pdf', 'image/jpeg', 'image/png', 'image/webp', 'text/plain' ), true );
		$inline = ! empty( $_GET['view'] ) && $previewable;
		...
		<div class="trb-portal__demo">
			...
			<details class="trb-portal__demo-module">
				...
				<form class="trb-portal__request-form trb-portal__demo-form" method="post" enctype="multipart/form-data" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" data-demo-form>
					...
					<fieldset data-demo-origins hidden style="min-width:0;border:1px solid #dce2ed;border-radius:12px;padding:16px;">
						...
						<label data-demo-origin-row="<?php echo esc_attr( $part ); ?>"><?php echo esc_html( $label ); ?> <span>*</span><select name="trb_demo_origin_<?php echo esc_attr( $part ); ?>" required><option value="">Seleziona la provenienza</option><?php foreach ( trb_demo_origin_options() as $key => $origin ) : ?><option value="<?php echo esc_attr( $key ); ?>"><?php echo esc_html( $origin ); ?></option><?php endforeach; ?></select></label>
						<?php endforeach; ?>
					</fieldset>
					<label data-demo-notes-block hidden>Cosa vuoi migliorare? <small>Facoltativo. Indica il tuo contributo, i dubbi, i passaggi da approfondire e l'eventuale uso dell'IA. Se voce e strumenti hanno provenienze diverse, specificalo qui.</small><textarea name="trb_demo_notes" maxlength="1500" rows="3" placeholder="Es. Ho scritto il testo; musica e voce sono una demo IA. Vorrei lavorare su immagini e metrica."></textarea></label>
					<div class="trb-portal__demo-upload" data-demo-text-block hidden>
						<label>Caricamento testo autoriale <span>*</span><small>Allega il testo da valutare in formato TXT o DOCX.</small><input type="file" name="trb_demo_text" accept=".txt,.docx,text/plain,application/vnd.openxmlformats-officedocument.wordprocessingml.document" data-demo-text /></label>
					</div>
					<div class="trb-portal__demo-upload" data-demo-audio-block hidden>
					...
					<p class="trb-portal__demo-rule" data-demo-material-hint role="status">Scegli il tipo di valutazione per proseguire.</p>
...
function trb_portal_submit_demo() {
	...
	// Discard irrelevant answers, including stale fields from a changed scope.
	foreach ( array('lyrics','music','performance') as $part ) {
		if ( ! in_array($part,trb_demo_scope_parts($review_context['focus']),true) ) unset($review_context[$part]);
	}
	...
	<div class="trb-release-files" id="release-files-<?php echo esc_attr( $release_id ); ?>">
		...
		<?php if ( $files_locked ) : ?>
			<div class="trb-portal__message trb-release-files__locked">
				...
				<p>Questa release ha superato l’approvazione: i file non possono più essere sostituiti autonomamente. Per richiedere una modifica, apri una segnalazione indicando il titolo della release e il file da valutare.</p>
				<a class="trb-button trb-button--compact" href="<?php echo esc_url( home_url( '/segnalazione/' ) ); ?>">Apri una segnalazione</a>
			</div>
		<?php endif; ?>
	...
	<template id="trb-portal-track-template">
		<article class="trb-portal__track" data-track>
			...
			<fieldset class="trb-portal__credits"><legend>Crediti</legend><p class="trb-portal__field-help">Inserisci ogni persona separatamente e seleziona tutti i ruoli che si applicano.</p>
				<?php if ($new_credits) { trb_symphonic_render_credits(); } else { ?>
...
function trb_portal_enqueue_assets() {
	...
	if ( is_front_page() || is_page( array( 'registrati', 'accedi', 'recupera-password', 'segnalazione' ) ) || is_singular( 'wpdmpro' ) || ( is_page() && $post && has_shortcode( $post->post_content, 'trb_artist_portal' ) ) ) {
		...
		wp_enqueue_script('trb-symphonic-credits',get_template_directory_uri().'/assets/js/trb-symphonic-credits.js',array(),(string)filemtime(get_template_directory().'/assets/js/trb-symphonic-credits.js'),true);
