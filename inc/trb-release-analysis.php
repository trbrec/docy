...
function trb_analysis_decide_release( $release_id ) {
	...
	if ( ! $table ) return;
	$rows = $wpdb->get_results( $wpdb->prepare( "SELECT track_index,payload,status FROM $table WHERE release_id=%d AND provider='acrcloud' AND service IN ('fingerprinting','fingerprinting_reuse') ORDER BY id ASC", $release_id ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
...
function trb_analysis_generate_report( $release_id ) {
	...
	if ( function_exists( 'trb_resource_tables' ) ) { global $wpdb; $ledger = trb_resource_tables()['usage']; $raw_rows = $wpdb->get_results( $wpdb->prepare( "SELECT track_index,payload FROM $ledger WHERE release_id=%d AND provider='acrcloud' AND service IN ('fingerprinting','fingerprinting_reuse') AND status='completed' ORDER BY id", $release_id ), ARRAY_A ); foreach ( $raw_rows as $raw_row ) { $lines[] = 'Risultato grezzo ACR brano ' . ( absint( $raw_row['track_index'] ) + 1 ) . ':'; foreach ( str_split( (string) $raw_row['payload'], 100 ) as $chunk ) $lines[] = $chunk; } }
