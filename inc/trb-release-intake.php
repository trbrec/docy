...
function trb_intake_record( $user_id, $token, $post ) {
	...
	try {
		...
		$tracks = trb_portal_sanitize_release_tracks( $raw_tracks, true );
		$meta = array(
...
function trb_intake_refresh_draft($id, $post) {
 ...
 if (isset($post['trb_tracks']) && is_array($post['trb_tracks'])) update_post_meta($id,'_trb_release_tracks',trb_portal_sanitize_release_tracks(array_slice($post['trb_tracks'],0,24,true),trb_symphonic_release($id)));
}
