...
if ( ! function_exists( 'csf_get_icons' ) ) {
  function csf_get_icons() {
    ...
    if ( ! empty( $icon_lists ) ) {
      ...
      echo '<div class="csf-error-text">'. esc_html__( 'No data available.', 'docy' ) .'</div>';
...
if ( ! function_exists( 'csf_export' ) ) {
  function csf_export() {
    ...
    if ( ! wp_verify_nonce( $nonce, 'csf_backup_nonce' ) ) {
      wp_die( esc_html__( 'Error: Invalid nonce verification.', 'docy' ) );
    }

    if ( ! current_user_can( 'manage_options' ) ) {
      wp_die( esc_html__( 'Error: You do not have permission to do that.', 'docy' ) );
    }

    if ( empty( $unique ) ) {
      wp_die( esc_html__( 'Error: Invalid key.', 'docy' ) );
    }
...
if ( ! function_exists( 'csf_chosen_ajax' ) ) {
  function csf_chosen_ajax() {
    ...
    if ( empty( $type ) || empty( $term ) ) {
      wp_send_json_error( array( 'error' => esc_html__( 'Error: Invalid term ID.', 'docy' ) ) );
    }
