...
check("connettore sincronizza artisti release e provini", all(token in CONNECTOR for token in ("'artist'", "'release'", "'demo'", "trb_crm_connector_profile_payload", "trb_crm_connector_release_payload", "trb_crm_connector_demo_payload")))
...
check("connettore effettua bootstrap incrementale", "trb_crm_connector_bootstrap" in CONNECTOR and all(token in CONNECTOR for token in ("'artists'", "'releases'", "'demos'", "'complete'")))
check("mutazioni CRM release autenticate e protette", "trb_crm_connector_verify_inbound" in CONNECTOR and "Una release con contratto firmato non può essere eliminata" in CONNECTOR and "hash_equals" in CONNECTOR)
