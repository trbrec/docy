...
DEMO_CONTEXT = (ROOT / "inc/trb-demo-context.php").read_text(encoding="utf-8")
...
check("release QA marcate e isolate da email e contratto reali", "_trb_release_qa_mode" in PORTAL and "trb_portal_release_is_qa" in PORTAL and "_trb_qa_artist_email_suppressed" in ANALYSIS and "qa_simulated" in BRIDGE)
check("account release QA non richiede documenti personali fittizi", "trb_portal_is_release_qa_account( $resolved_user )" in PORTAL and "Artista QA Collection 24" in BRIDGE)
...
check("spotify9 viene normalizzato sul ruolo TRB", "'spotify9' => array( 'profile' => 'trb'" in BRIDGE and "'spotify9' === $login" in BRIDGE and "$user->add_role( 'artista_d' )" in BRIDGE)
...
check("coda email conserva CC e Reply-To validati", "headers longtext NULL" in RESOURCE and "Cc|Reply-To" in RESOURCE and "json_decode( (string) ( $row->headers" in RESOURCE)
...
check("ripubblicazione dichiarata salta ACR solo con tutti gli ISRC validi", "trb_analysis_catalogue_bypass_eligible" in ANALYSIS and "catalogue_reissue" in ANALYSIS and "count( $tracks ) !== count( $declarations )" in ANALYSIS and "trb_analysis_apply_catalogue_bypass" in RESOURCE)
check("bypass ACR registrato in decisione report e cronologia", "ACRCLOUD_BYPASSED_DECLARED_CATALOGUE_REISSUE" in ANALYSIS and "declared_catalogue_reissue" in ANALYSIS and "acrcloud_bypassed_declared_catalogue_reissue" in ANALYSIS and "ACRCloud: non eseguito" in ANALYSIS)
...
check("monitor giornaliero distingue capacità, collegamenti e credito non verificato", "pcloud_warning_1" in RESOURCE and "temp_min_free_bytes" in RESOURCE and "'acr-connection'" in RESOURCE and "'acr-budget', trb_resource_acr_budget_notice" not in RESOURCE and "lettura dello spazio da collegare" in RESOURCE)
...
check("pCloud prova WebDAV dopo API e mostra codice diagnostico", "PCLOUD_API_QUOTA_UNAVAILABLE" in RESOURCE and "$data = trb_resource_pcloud_webdav_userinfo();" in RESOURCE and "trb_resource_pcloud_diagnostic" in RESOURCE and "strtoupper( $pcloud_code )" in RESOURCE)
...
check("cestino release ferma polling retry e nuove spese", "trb_owner_dashboard_stop_release_jobs" in OWNER and "cancelled_by_owner" in OWNER and "wp_clear_scheduled_hook( 'trb_resource_poll_acr_job'" in OWNER and "TRB_RELEASE_CANCELLED" in RESOURCE and "'cancelled' === $row->status" in RESOURCE)
check("record con contratto inviato o firmato non è cestinabile", "array( 'contract_sent', 'signed' )" in OWNER and "deve restare nello storico" in OWNER)
...
check("valutazione demo separa analisi audio e testo", "trb_demo_review_prompt" in DEMO and "Non hai audio da ascoltare" in DEMO_CONTEXT and "Non hai un testo allegato" in DEMO_CONTEXT)
check("genere demo obbligatorio usa tassonomia canonica", 'name="trb_demo_genre"' in PORTAL and "in_array( $genre, trb_portal_genres(), true )" in PORTAL)
check("genere demo salvato e incluso nell'idempotenza", "'genre' => $genre" in PORTAL and "strtolower( $genre )" in PORTAL)
check("genere demo orienta OpenAI senza imporre stereotipi", "genere_dichiarato" in DEMO_CONTEXT and "Il genere dichiarato è un riferimento, non una prova" in DEMO_CONTEXT)
check("genere demo sincronizzato e mostrato nella valutazione", "'genere' => $payload['genre'] ?? ''" in DEMO and "$genre_html" in DEMO and "Genere musicale:</strong>" in DEMO)
...
check("connettore usa coda persistente e conferma per evento", "trb_crm_sync_outbox" in CONNECTOR and "'acknowledged'" in CONNECTOR and "'retry'" in CONNECTOR and "event_id" in CONNECTOR)
check("connettore firma ogni lotto con HMAC", "hash_hmac( 'sha256', $timestamp . '.' . $body" in CONNECTOR and "X-TRB-Signature" in CONNECTOR)
check("connettore non esporta segreti o percorsi locali", "private.?key|api.?key|local.?path|tmp.?name" in CONNECTOR and "array_intersect_key" in CONNECTOR)
check("connettore gira ogni cinque minuti", "5 * MINUTE_IN_SECONDS" in CONNECTOR and "trb_five_minutes" in CONNECTOR and "schedule_seconds' => 300" in CONNECTOR)
check("connettore effettua bootstrap incrementale", "trb_crm_connector_bootstrap" in CONNECTOR and all(token in CONNECTOR for token in ("'artists'", "'demos'", "'complete'")))
check("health check connettore disponibile", "'/crm-sync-health'" in CONNECTOR and "trb_crm_connector_health" in CONNECTOR)
